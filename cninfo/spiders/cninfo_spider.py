...
class CninfoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        try:
            ...
            items = []
            while (flag):
                logger.info('--------- current is %d page  ---------' % self.page_num)
                random_randint = random.randint(60, 90)
                time.sleep(random_randint)
                logger.info('---------- sleep time %ds,next page---------- ' % random_randint)
                # TODO 防止页面没有加载完成，
                self.driver.implicitly_wait(30)
                flag, items = self.get_info(flag, items, public_main_window)

            # TODO 切换到主页
            self.driver.switch_to_window(public_main_window)
            # TODO 更新公司概况，公司存在就不更新，否则会影响uuid的值，无法匹配公司公告
            ...
            self.mysql_util.exec_db_cmd(insert_sql)
    ...
    def get_info(self, flag, items, public_main_window):
        """
        1,获取公司概况
        2,获取公告信息
        3,下载公告信息
                        ...
                        if class_text == 'g1':
                            ...
                            if dds:
                                ...
                                for j in range(1, len(dds) + 1):
                                    ...
                                    # print 'date=%s,title = %s,type = %s,link = %s,filename = %s' % (
                                    # date, title, type, link, filename)
    ...
    def download_data(self, a_tag, public_main_window):
        ...
        time.sleep(rr)
        logger.info('---------- random sleep %d , wait download complete ----------' % rr)
        # 下载条数更新
